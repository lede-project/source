...
FDISK=$(env PATH="/usr/local/sbin:/usr/sbin:/sbin:$PATH" which fdisk) || exit 1

# generate image file
printf "Creating $OUTFILE from /dev/zero: "
dd if=/dev/zero of="$OUTFILE" bs=512 count=0 seek=$IMGSIZE >/dev/null || exit 1
printf "Done\n"
...
dd of="$OUTFILE" if="$BOOTLOADER" bs=512 seek=1 conv=notrunc status=none
printf "Done\n"
...
for i in $(seq 1 1 $NUMPARTS); do
	...
	dd if="$img" of="$OUTFILE" bs=512 seek=$offset conv=notrunc status=none
	printf "Done\n"
